...
code_seq gen_code_if_stmt(if_stmt_t stmt)
{
    ...
    code_seq condCode = gen_code_expr(stmt.expr);

    // Push the truth value of the condition onto the stack
    condCode = code_seq_add_to_end(condCode, code_push_reg_on_stack(V0));

    // Pop the top of the stack into $v0
    condCode = code_seq_add_to_end(condCode, code_pop_stack_into_reg(V0));

    // Generate code for S1
    code_seq thenCode = gen_code_stmt(*(stmt.then_stmt));

    // BEQ $0, $v0, elseCode
    condCode = code_seq_concat(condCode, code_seq_singleton(code_beq(0, V0, code_seq_size(thenCode) + 2)));

    // Concatenate code for S1
    condCode = code_seq_concat(condCode, thenCode);

    // Generate code for S2
    code_seq elseCode = gen_code_stmt(*(stmt.else_stmt));

    // Concatenate code for S2
    condCode = code_seq_concat(condCode, elseCode);
...
code_seq gen_code_condition(condition_t cond)
{
    ...
    switch (cond.cond_kind) {
        case ck_odd: {
            ...
            odd_condition_t oddCond = cond.data.odd_cond;
            expr_t expr = oddCond.expr;

            // Generate code for the expression
            code_seq exprCode = gen_code_expr(expr);

            // Combine the generated code sequences
            conditionCode = code_seq_concat(conditionCode, exprCode);

            // Pop the top of the stack into a temporary register
            conditionCode = code_seq_concat(conditionCode, code_seq_singleton(code_pop_stack_into_reg(V0)));

            // Check if the value in V0 is odd
            conditionCode = code_seq_concat(conditionCode, code_seq_singleton(code_andi(V0, V0, 1))); // V0 = V0 & 1

            // Push the result of the odd check back onto the stack
        ...
        case ck_rel: {
            ...
            code_seq rightCode = gen_code_expr(relOpCond.expr2);

            // Combine the generated code sequences
            conditionCode = code_seq_concat(conditionCode, leftCode);
            conditionCode = code_seq_concat(conditionCode, rightCode);

            // Pop the top of the stack into temporary registers
            conditionCode = code_seq_concat(conditionCode, code_seq_singleton(code_pop_stack_into_reg(V0)));
